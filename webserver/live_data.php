...
$current_time = time();
//echo date("Y-m-d h:i",$current_time);

$enddate = date("Y-m-d H:i");
$startdate = strtotime('-12 hour',strtotime($enddate));
$end_date = explode(" ",$enddate)[0];
$end_time = explode(" ",$enddate)[1];
$s_date = date("Y-m-d H:i",$startdate);
$start_date = explode(" ",$s_date)[0];
$start_time = explode(" ",$s_date)[1];

$site_select = 1;

//echo $site_select;
//echo $end_time;
$s_query = "SELECT site_name FROM site WHERE site_id = '$site_select'";
$s_data = mysqli_query($db,$s_query);
$site_name = mysqli_fetch_assoc($s_data);
		...
		elseif($sensor_type[$i] == 'AQ')
		{
			...
			$data_result = mysqli_query($db,$data_query);
				...
				while($row=mysqli_fetch_assoc($data_result)) {
					...
					//echo $row['data_date'];
					array_push($dataPoints1, array("x" => $js_ts, "y" => $row['pm1']));
					array_push($dataPoints2, array("x" => $js_ts, "y" => $row['pm25']));
...
<div class = "form-container">
  <form action="live-data" method="POST">
  ...
  <div class="row">
    ...
    <div class="col-75">
	  ...
	  <option value = <?php echo "$site_id[0]"?> selected><?php echo $site_list[0]?></option>
